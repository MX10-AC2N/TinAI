...
# ══════════════════════════════════════════════════════════════════
#  TinAI v2 – Installateur ARM64 (aarch64)
#  Usage : bash install-arm64.sh
#  Compile llama.cpp et llama-swap nativement (NEON/SVE)
...
if [ "$(uname -m)" != "aarch64" ]; then
    echo "❌ Ce script est réservé aux machines ARM64 (aarch64)"
    echo "   Sur x86_64, utilise : bash install.sh"
    exit 1
fi
...
echo "🧠 TinAI v${TINAI_VERSION} – ARM64 (aarch64)"
echo "════════════════════════════════════════════════"
...
# ════════════════════════════════════════════════════════════════
#  Build des images ARM64 natives (avec cache)
# ════════════════════════════════════════════════════════════════

# ── llama.cpp ARM64 ───────────────────────────────────────────────
# ghcr.io/ggml-org/llama.cpp:server n'a PAS de build ARM64 natif.
# On compile depuis les sources avec NEON/SVE activé (~5-10 min).
LLAMA_IMAGE="tinai-llamacpp:local"

if docker image inspect tinai-llamacpp:local >/dev/null 2>&1; then
    echo "✅ llama.cpp ARM64 déjà compilé (cache)"
else
    echo "🔨 Compilation llama.cpp (NEON/SVE) – ~5-10 min..."
    docker build -t tinai-llamacpp:local - << 'LLAMADOCKERFILE'
...
LLAMADOCKERFILE
    echo "✅ llama.cpp ARM64 compilé"
fi

# ── llama-swap ARM64 ──────────────────────────────────────────────
# ghcr.io/mostlygeek/llama-swap:cpu = x86 only.
# Build Go natif CGO_ENABLED=0 (~1-2 min).
SWAP_IMAGE="tinai-llamaswap:local"

if docker image inspect tinai-llamaswap:local >/dev/null 2>&1; then
    echo "✅ llama-swap ARM64 déjà compilé (cache)"
else
    echo "🔨 Compilation llama-swap ARM64 – ~1-2 min..."
    docker build -t tinai-llamaswap:local - << 'SWAPDOCKERFILE'
FROM golang:1.23-bookworm AS builder
RUN git clone --depth=1 https://github.com/mostlygeek/llama-swap /src
WORKDIR /src
RUN CGO_ENABLED=0 GOARCH=arm64 go build -ldflags="-s -w" -o /llama-swap .
FROM debian:bookworm-slim
RUN apt-get update && apt-get install -y --no-install-recommends curl ca-certificates \
    && rm -rf /var/lib/apt/lists/*
COPY --from=builder /llama-swap /usr/local/bin/llama-swap
ENTRYPOINT ["/usr/local/bin/llama-swap"]
SWAPDOCKERFILE
    echo "✅ llama-swap ARM64 compilé"
fi

# ── ComfyUI ARM64 ─────────────────────────────────────────────────
# yanwk/comfyui-boot:cpu = x86 only → ai-dock/comfyui multi-arch
COMFYUI_IMAGE="ai-dock/comfyui:latest-cpu"
...
# SillyTavern : start_period 120s + retries 20 (Node v24 ARM64 lent au 1er boot)
...
# ════════════════════════════════════════════════════════════════
#  Génération du CLI tinai (ARM64, services sélectionnés)
# ════════════════════════════════════════════════════════════════
...
#  Généré le $(date -u +"%Y-%m-%d %H:%M UTC") par install-arm64.sh
#  Architecture : ${ARCH} | Images : natives (tinai-llamacpp:local)
# ══════════════════════════════════════════════════════════════════
...
cmd_update() {
    ...
    echo "ℹ️  Pour reconstruire llama.cpp ARM64 : docker rmi tinai-llamacpp:local && bash install-arm64.sh"
}
...
cmd_model() {
    ...
    echo "   Image locale  : \${CYAN}\${LLAMA_IMAGE}\${RESET}"
    echo ""
    declare -A MODELS=(
    ...
    if command -v gum &>/dev/null; then
        ...
        read -rp "Numéro : " NUM; SELECTED="\${MODEL_NAMES[\$((NUM-1))]}"
    fi
    [ -z "\$SELECTED" ] && exit 0
...
cmd_stop()    { check_compose; cd "\$TINAI_DIR"; echo -e "\${YELLOW}⏹️  Arrêt...\${RESET}"; docker compose down; echo -e "\${GREEN}✅ Arrêté\${RESET}"; }
...
cmd_help() {
    ...
    echo "  \${CYAN}status\${RESET}          État des services installés"
    echo "  \${CYAN}logs [service]\${RESET}  Logs en direct"
    echo "  \${CYAN}update\${RESET}          Met à jour les images Docker"
    echo "  \${CYAN}model\${RESET}           Change le modèle LLM à chaud"
    ...
    echo ""
    echo "ℹ️  ARM64 : llama.cpp compilé nativement → tinai-llamacpp:local"
    echo "   Rebuild : docker rmi tinai-llamacpp:local && bash install-arm64.sh"
}
